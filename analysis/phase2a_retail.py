...
def main() -> None:
    ...
    metrics_path = OUTPUTS_DIR / "phase2a_retail_metrics.csv"
    pct_df.to_csv(metrics_path, index=False)
    print(f"Saved: {metrics_path}")

    traj_path = OUTPUTS_DIR / "phase2a_retail_trajectories.png"
    plot_trajectories(pct_df, traj_path)
    print(f"Saved: {traj_path}")

    scoreboard_path = OUTPUTS_DIR / "phase2a_retail_scoreboard.png"
    score_t0 = plot_scoreboard(pct_df, scoreboard_path)
    print(f"Saved: {scoreboard_path}")

    summary = hit_rate_summary(pct_df, score_t0)
    summary_path = OUTPUTS_DIR / "phase2a_retail_summary.csv"
